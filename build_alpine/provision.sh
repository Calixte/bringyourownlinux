...
# Create minimal network config; the kernel cmdline net.ifnames=0 biosdevname=0
# ensures predictable eth0 naming, and DHCP on eth0 gives immediate connectivity.
...
GRUB_CMDLINE_LINUX="rootfstype=ext4 net.ifnames=0 biosdevname=0 nomodeset iommu=pt"
...
# Wipe cloud-init state so it treats the target hardware as a new instance
# and processes the ConfigDrive for SSH keys and network config.
cloud-init clean --logs

# Prioritise the OVH ConfigDrive over any NoCloud / cidata seed.
mkdir -p /etc/cloud/cloud.cfg.d
cat > /etc/cloud/cloud.cfg.d/99_ovh.cfg <<'EOF'
